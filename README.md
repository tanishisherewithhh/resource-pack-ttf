...
This Python script converts Minecraft: Java Edition [font definition files](https://minecraft.wiki/w/Font#Providers) and resource pack files (`.zip` or `folder`) to [TrueType Fonts (TTFs)](https://en.wikipedia.org/wiki/TrueType).
...
1. [Download and extract the repo](https://github.com/tanishisherewithhh/resource-pack-ttf/archive/refs/heads/master.zip) or clone it (`git clone https://github.com/tanishisherewithhh/resource-pack-ttf`)
2. Place your resource pack that contains the font assets in the `resource_packs` folder. `.zip` or folder is used. `.rar` will not work.
3. Install [`uv`](https://docs.astral.sh/uv/#installation)
4. `uv run main.py`
5. Fonts will be created in the `out` folder
